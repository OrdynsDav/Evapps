import { FormProvider, useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import type { FormProps } from "../interfaces";
import "./Form.css";
import { FormActions } from "./FormActions/FormActions";
import { FormInputs } from "./FormInputs/FormInputs";
import type { z } from "zod";
import { useEffect, useMemo, useState } from "react";
import { useAuth } from "../../api/hooks";
import { useNavigate } from "react-router-dom";

export function Form({
    inputs,
    buttons,
    title,
    upload,
    schema,
    type,
}: FormProps) {
    type FormData = z.infer<typeof schema>;

    const useFormReturn = useForm<FormData>({
        resolver: zodResolver(schema),
        mode: "onSubmit",
        defaultValues: {
            email: "test@test.com",
            password: "12345678",
            verifyPassword: "12345678",
        },
    });

    const {
        handleSubmit,
        formState: { errors, isSubmitting },
        getValues,
    } = useFormReturn;

    const [serverError, setServerError] = useState<{
        field: string;
        message: string;
        global?: boolean;
    } | null>(null);

    useEffect(() => {
        setServerError(null);
    }, []);

    const auth =
        type === "login" || type === "register"
            ? useAuth(type, setServerError)
            : { mutate: null };
    const navigate = useNavigate();

    const onSubmit = async () => {
        const actualData = getValues();

        switch (type) {
            case "register":
            case "login":
                if (auth.mutate) {
                    auth.mutate({
                        email: actualData.email as string,
                        password: actualData.password as string,
                    }, {
                        onSuccess: () => navigate("/")
                    });
                }
                break;
            default:
                break;
        }
    };

    const allErrors = useMemo(
        () => ({
            ...errors,
            ...(serverError?.field === "email"
                ? { email: { message: serverError.message } }
                : {}),
        }),
        [errors, serverError]
    );

    return (
        <FormProvider {...useFormReturn}>
            <form className="form" onSubmit={handleSubmit(onSubmit)}>
                <h2 className="form__title">{title}</h2>
                {serverError?.global ? 
                    <p className="form__error">{serverError.message}</p>
                : null}
                {upload && (
                    <button className="form__btn" type="button">
                        <svg
                            width="14"
                            height="14"
                            viewBox="0 0 14 14"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                d="M5.24988 3.96881H6.40457V9.25475C6.40457 9.3235 6.46082 9.37975 6.52957 9.37975H7.46707C7.53582 9.37975 7.59207 9.3235 7.59207 9.25475V3.96881H8.74988C8.85457 3.96881 8.91238 3.8485 8.84832 3.76725L7.09832 1.55162C7.08662 1.53668 7.07168 1.5246 7.05463 1.51629C7.03757 1.50798 7.01885 1.50366 6.99988 1.50366C6.98091 1.50366 6.96218 1.50798 6.94513 1.51629C6.92807 1.5246 6.91313 1.53668 6.90144 1.55162L5.15144 3.76569C5.08738 3.8485 5.14519 3.96881 5.24988 3.96881ZM12.7186 8.786H11.7811C11.7124 8.786 11.6561 8.84225 11.6561 8.911V11.3172H2.34363V8.911C2.34363 8.84225 2.28738 8.786 2.21863 8.786H1.28113C1.21238 8.786 1.15613 8.84225 1.15613 8.911V12.0047C1.15613 12.2813 1.37957 12.5047 1.65613 12.5047H12.3436C12.6202 12.5047 12.8436 12.2813 12.8436 12.0047V8.911C12.8436 8.84225 12.7874 8.786 12.7186 8.786Z"
                                fill="black"
                                fill-opacity="0.25"
                            />
                        </svg>
                        <span className="form__btn-text">
                            Загрузите ваше фото
                        </span>
                    </button>
                )}
                <FormInputs inputs={inputs} errors={serverError?.global ? null : allErrors} />
                <FormActions buttons={buttons} isSubmitting={isSubmitting} />
            </form>
        </FormProvider>
    );
}
